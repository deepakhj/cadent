// from https://github.com/youtube/vitess/blob/master/go/ratelimiter/ratelimiter.go
...
// Allow returns true if a request is within the rate limit norms for "num" items
// Otherwise, it returns false.
func (rl *RateLimiter) AllowMany(num int) bool {
	rl.mu.Lock()
	defer rl.mu.Unlock()
	if time.Now().Sub(rl.lastTime) < rl.interval {
		if rl.curCount-num >= 0 {
			rl.curCount -= num
			return true
		}
		return false
