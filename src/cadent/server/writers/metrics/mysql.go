...
func (my *MySQLMetrics) Flush() (int, error) {
	...
	Q := fmt.Sprintf(
		"INSERT INTO %s (stat, sum, mean, min, max, last, count, resolution, time) VALUES ",
		my.db.Tablename(),
	)
	...
	for _, stat := range my.write_list {
		Q += "(?,?,?,?,?,?,?,?,?), "
		vals = append(
			vals, stat.Key, stat.Sum, stat.Mean, stat.Min, stat.Max, stat.Last, stat.Count, stat.Resolution, stat.Time,
		)
