...
/*
	...
	TagMode ::
		...
		If `tag_mode="metrics2"` we obey use the UniqueIDs as generated by <intrinsic tags> only
		if `tag_mode="all"`  then that we use all the tags assigned to make the Unique ID
...
func (wc *Cacher) SetTagMode(m string) {
	wc.TagMode = m
}
...
func (wc *Cacher) Add(name *repr.StatName, stat *repr.StatRepr) error {
	...
	// set the tag mode for the uid generation
	name.SetTagMode(wc.TagMode)
