...
func TestSimpleJsonTimeSeries(t *testing.T) {
	...
	Convey("SimpleJsonTimeSeries", t, func() {
		stat := repr.StatRepr{
			Key:        "goo",
			StatKey:    "goo",
			Sum:        5,
		...
		max_size := 8 * 1024
		ser_size := NewSimpleJsonTimeSeries(n.UnixNano())
		on_tick := 0
		for true {
			on_tick++
...
func BenchmarkJsonSeriesPut(b *testing.B) {
	b.ResetTimer()
	stat := repr.StatRepr{
		Sum:        5,
		Min:        1,
...
func BenchmarkJsonSeriesCompress(b *testing.B) {
	b.ResetTimer()
	stat := repr.StatRepr{
	...
	for i := 0; i < b.N; i++ {
		...
		b.Logf("Comp Data Len: Pre: %v,  %v bytes per stat %v", pre_l, len(c_bss), len(c_bss)/n_stat)
	}
}
