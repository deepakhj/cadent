...
func TestProtbufArrayTimeSeries(t *testing.T) {
	...
	Convey("SimpleProtobufArraySeries", t, func() {
		stat := repr.StatRepr{
			Key:        "goo",
			StatKey:    "goo",
			Sum:        5,
		...
		ser := NewSimpleProtobufTimeSeries(n.UnixNano())
		var err error
		n_stats := 10
		...
		dec, _ := snappy.Decode(nil, bss)
		new_ser := NewSimpleProtobufTimeSeries(n.UnixNano())
		new_ser.UnmarshalBinary(dec)
		it, err = new_ser.Iter()
		idx = 0
		for it.Next() {
			r := it.ReprValue()
		...
		max_size := 8 * 1024
		ser_size := NewSimpleProtobufTimeSeries(n.UnixNano())
		on_tick := 0
		for true {
			on_tick++
...
func BenchmarkProtobufSeriesPut(b *testing.B) {
	b.ResetTimer()
	stat := repr.StatRepr{
		Sum:        5,
		Min:        1,
...
func BenchmarkProtobufSeriesSnappyCompress(b *testing.B) {
	b.ResetTimer()
	stat := repr.StatRepr{
...
func BenchmarkProtobufSeriesFlateCompress(b *testing.B) {
	b.ResetTimer()
	stat := repr.StatRepr{
		Sum:   repr.JsonFloat64(rand.Float64()),
		Min:   repr.JsonFloat64(rand.Float64()),
