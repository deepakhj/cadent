...
func (s *ReprTimeSeries) AddPoint(t int64, min float64, max float64, first float64, last float64, sum float64, count int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (it *ReprIter) Values() (int64, float64, float64, float64, float64, float64, int64) {
	return it.curStat.Time.UnixNano(),
		float64(it.curStat.Min),
		float64(it.curStat.Max),
		float64(it.curStat.First),
